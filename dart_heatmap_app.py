...
▸ Gold ★ shows optimal aim
▸ Dashed ellipses (1 σ, 2 σ) show where darts land when aiming there
▸ Sliders:
    • **Timing spread σₜ** – error *along* the throwing arm (early/late release)
    • **Lateral spread σₗ** – error *across* the arm (left/right miss)
    • Arm angle defined relative to the board (0° = straight up through 20)
"""
...
# Rotate display so 20 sits at 12 o’clock
...
# ---------------- EV grid -----------------------------
def ev_grid(sig_timing, sig_lateral, theta_arm_board_deg,
            n_samples, r_grid=None, th_grid=None, seed=1234):
    """Return EV grid and the *world* arm angle (radians)."""
    if r_grid is None:
        r_grid = np.arange(0.0, R_DOUBLE_IN, 4.0)
    if th_grid is None:
        th_grid = np.arange(0.0, 360.0, 4.0)

    # Aim mesh (world coords)
    ...
    eps_timing = rng.normal(0, sig_timing, (n_samples, 1))
    eps_lat    = rng.normal(0, sig_lateral, (n_samples, 1))

    xs = x0.ravel()[None, :] + eps_timing * ux + eps_lat * vx
    ys = y0.ravel()[None, :] + eps_timing * uy + eps_lat * vy
...
if st.button("Compute", type="primary"):
    with st.spinner("Simulating darts…"):
        ev, r_mm, th_deg, world_arm_rad = ev_grid(
            sig_timing, sig_lateral, theta_arm_board, int(N)
        )
