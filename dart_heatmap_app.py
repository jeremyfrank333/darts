...
▸ Dashed line shows the arm axis through the ★
▸ Angle slider 0–180 ° (axis bidirectional) — dial preview removed
"""
...
SECTOR_VALUES = np.array([20,1,18,4,13,6,10,15,2,17,3,19,7,16,8,11,14,9,12,5])
ROT_SHIFT = np.pi/2 - np.pi/20   # rotate so 20 is up
...
# --- EV grid ---
def ev_grid(sig_t, sig_l, theta_board_deg, N,
...
if st.button("Compute", type="primary"):
    ...
    pcm = ax.pcolormesh(Th_e, R_e, ev.T, cmap='plasma', shading='auto',
                        alpha=0.75, zorder=2)
